...
for index, row in cleaned_data.iterrows():
    user_reviews_dict[row['reviewer_id']].append(row.to_dict())
...
def build_global_product_counts(users):
    ...
    for user_id, reviews in users:
        ...
        product_count[first_review['product_id']].append(user_id)  # Track which users reviewed the first product
...
def set_random_first_review_with_global_neighbor(user_reviews, global_product_counts):
    ...
    valid_reviews = [review for review in user_reviews if len(global_product_counts[review['product_id']]) > 1]
...
def add_user_to_split(reviewer_id, user_reviews, split_users, local_product_counts, added_users):
    ...
    for review in user_reviews:
        local_product_counts[review['product_id']] += 1
...
def process_user_data(users):
    ...
    for user_id, reviews in tqdm.tqdm(users):
        ...
        pred_entry = {"id": user_id, "output": reviews[0]['review_title']}
